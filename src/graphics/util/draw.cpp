...
REXCVAR_DEFINE_BOOL(
    depth_bias_shader_offset, false, "GPU",
...
bool GetResolveInfo(const RegisterFile& regs, const memory::Memory& memory,
  ...
  // The caller checks width/height and skips the resolve. Some titles (Forza
  // Horizon 1/2) legitimately hit this a lot without any visible rendering
  // impact - erroring here just spams the log and hides real resolve bugs.
  if (x0 >= x1 || y0 >= y1) {
...
ResolveCopyShaderIndex ResolveInfo::GetCopyShader(uint32_t draw_resolution_scale_x,
  ...
  // fixed interpretation, full resolve has to do the repack. If copy_dest_number
  // asks for a fixed interpretation that'd be decoded to linear by a real
  // hardware resolve, it needs the full shader conversion. Any title keeping the
