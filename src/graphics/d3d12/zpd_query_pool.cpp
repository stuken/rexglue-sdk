...
bool ZPDQueryPool::EnsureInitialized(const rex::ui::d3d12::D3D12Provider& provider,
  ...
  if (!rtv_initialized()) {
    ...
    if (FAILED(device->CreateQueryHeap(&heap_desc, IID_PPV_ARGS(&query_heap_)))) {
      REXGPU_WARN("ZPDQueryPool: Failed to create the ZPD query heap, "
                  "falling back to fake sample counts.");
      return false;
    }

    D3D12_RESOURCE_DESC buffer_desc;
    rex::ui::d3d12::util::FillBufferResourceDesc(buffer_desc, sizeof(uint64_t) * requested_capacity,
    ...
    if (FAILED(device->CreateCommittedResource(&rex::ui::d3d12::util::kHeapPropertiesReadback,
      ...
      REXGPU_WARN("ZPDQueryPool: Failed to allocate the ZPD query "
    ...
    if (FAILED(readback_buffer_->Map(0, &read_range, &mapping))) {
      REXGPU_WARN("ZPDQueryPool: Failed to map the ZPD query "
                  "readback buffer, falling back to fake sample counts.");
      Shutdown();
      return false;
    }
  ...
  if (FAILED(device->CreateCommittedResource(&rex::ui::d3d12::util::kHeapPropertiesDefault,
    ...
    REXGPU_WARN("ZPDQueryPool: Failed to create the ZPD ROV counter "
  ...
  if (FAILED(device->CreateCommittedResource(&rex::ui::d3d12::util::kHeapPropertiesReadback,
                                             provider.GetHeapFlagCreateNotZeroed(),
                                             &readback_buffer_desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                             nullptr, IID_PPV_ARGS(&rov_counter_readback_buffer_)))) {
    REXGPU_WARN("ZPDQueryPool: Failed to create the ZPD ROV counter readback "
                "buffer, falling back to fake sample counts.");
    rov_counter_buffer_.Reset();
    return false;
  }
  ...
  if (FAILED(rov_counter_readback_buffer_->Map(0, &read_range, &mapping))) {
    REXGPU_WARN("ZPDQueryPool: Failed to map the ZPD ROV counter readback "
                "buffer, falling back to fake sample counts.");
    rov_counter_readback_buffer_.Reset();
    rov_counter_buffer_.Reset();
    return false;
...
bool ZPDQueryPool::AcquireQueryIndex(uint32_t& query_index, uint32_t& query_generation) {
...
void ZPDQueryPool::ReleaseQueryIndex(uint32_t query_index, uint32_t query_generation) {
...
bool ZPDQueryPool::GenerationMatches(uint32_t query_index, uint32_t query_generation) const {
  return query_index < index_generations_.size() &&
         index_generations_[query_index] == query_generation;
}

void ZPDQueryPool::BeginQuery(DeferredCommandList& deferred_command_list,
...
void ZPDQueryPool::EndQuery(DeferredCommandList& deferred_command_list, uint32_t query_index) const {
  if (!query_heap_ || query_index >= capacity_) {
    return;
  }
...
void ZPDQueryPool::QueueQueryResolve(uint32_t query_index, bool uses_rov_counter) {
  if (query_index >= capacity_) {
    return;
  }
...
void ZPDQueryPool::ClearROVCounter(DeferredCommandList& deferred_command_list,
                                   uint32_t query_index) const {
...
void ZPDQueryPool::FlushResolveBatch(DeferredCommandList& deferred_command_list,
...
uint64_t ZPDQueryPool::GetQueryReadbackValue(uint32_t query_index, bool uses_rov_counter) const {
  if (query_index >= capacity_) {
    return 0;
  }
