...
void SpirvShaderTranslator::FSI_LoadEdramOffsets(spv::Id msaa_samples) {
  ...
  for (uint32_t i = 0; i < 2; ++i) {
    ...
    if (resolution_scale[i] > 1) {
      ...
      guest_pixel[i] =
          builder_->createBinOp(spv::OpUDiv, type_uint_, host_pixel, const_scale);
      guest_subpixel[i] =
          builder_->createBinOp(spv::OpUMod, type_uint_, host_pixel, const_scale);
    } else {
...
spv::Id SpirvShaderTranslator::FSI_AddSampleOffset(spv::Id sample_0_address, uint32_t sample_index,
  ...
  if ((sample_index & 1) && is_64bpp != spv::NoResult) {
    ...
    sample_offset = builder_->makeIntConstant(int32_t(sample_row_offset + sample_column_offset_32bpp));
  }
...
spv::Id SpirvShaderTranslator::PackFloat16x2ExtendedRange(spv::Id float2_value) {
  ...
  float2_value = builder_->createTriOp(spv::OpSelect, type_float2_,
  ...
  spv::Id clamped = builder_->createTriBuiltinCall(type_float2_, ext_inst_glsl_std_450_,
                                                   GLSLstd450FClamp, float2_value, const_neg_131008,
                                                   const_131008);
  id_vector_temp_.clear();
  ...
  std::array<spv::Id, 2> packed_16_float;
  {
    ...
    for (uint32_t i = 0; i < 2; ++i) {
      ...
      packed_16_float[i] = PackFloat16x2ExtendedRange(
          builder_->createRvalueSwizzle(spv::NoPrecision, type_float2_, color_float4,
                                        uint_vector_temp_));
    }
    builder_->createBranch(&block_format_merge);
  }
