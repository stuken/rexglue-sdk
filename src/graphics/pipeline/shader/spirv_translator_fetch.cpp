        ...
        case xenos::FetchOpDimension::kCube: {
          ...
          if (instr.dimension == xenos::FetchOpDimension::k1D) {
            ...
            spv::Id width_1d_minus_1 = builder_->createTriOp(
                spv::OpBitFieldUExtract, type_uint_, fetch_constant_word_2, const_uint_0_,
                builder_->makeUintConstant(xenos::kTexture1DMaxWidthLog2));
            size[0] = builder_->createTriOp(spv::OpSelect, type_uint_, data_is_1d,
      ...
      if (selected_mip_grid_possible) {
        ...
        spv::Id fetch_constant_word_4_mips = builder_->createLoad(
            builder_->createAccessChain(spv::StorageClassUniform, uniform_fetch_constants_,
                                        id_vector_temp_),
            spv::NoPrecision);
        selected_mip_level = builder_->createTriOp(spv::OpBitFieldUExtract, type_uint_,
                                                   fetch_constant_word_4_mips,
                                                   builder_->makeUintConstant(2),
                                                   builder_->makeUintConstant(4));
        spv::Id mip_max_level = builder_->createTriOp(spv::OpBitFieldUExtract, type_uint_,
                                                      fetch_constant_word_4_mips,
          ...
          if (selected_mip_locked != spv::NoResult) {
            ...
            spv::Id selected_mip_size = builder_->createBinBuiltinCall(
                ...
                builder_->createBinOp(spv::OpShiftRightLogical, type_uint_, size_component_ref,
                                      selected_mip_level),
                builder_->makeUintConstant(1));
            size_component_ref = builder_->createTriOp(spv::OpSelect, type_uint_, selected_mip_locked,
      ...
      if (instr.dimension == xenos::FetchOpDimension::k1D &&
        ...
        spv::Id data_dimension_1d =
            builder_->createTriOp(spv::OpBitFieldUExtract, type_uint_, fetch_constant_word_5_for_1d,
                                  builder_->makeUintConstant(9), builder_->makeUintConstant(2));
        spv::Id is_actually_1d =
            builder_->createBinOp(spv::OpIEqual, type_bool_, data_dimension_1d,
                                  builder_->makeUintConstant(
                                      static_cast<unsigned int>(xenos::DataDimension::k1D)));
        ...
        spv::Id is_wide_1d = builder_->createBinOp(spv::OpLogicalAnd, type_bool_, is_actually_1d,
                                                   is_wide);
        ...
        spv::Id coord_x_wide, coord_y_wide;
        {
          ...
          // normalized_x = x_in_row / row_width
          coord_x_wide = builder_->createNoContractionBinOp(spv::OpFDiv, type_float_, x_in_row,
                                                            row_width_float);
          // normalized_y = (row_index + 0.5) / num_rows - sample the middle of
          // the row. (row_index / num_rows) * num_rows rounds to just below
          // row_index.
        ...
        if (use_computed_lod && !use_lod_bias) {
          ...
          switch (instr.dimension) {
            case xenos::FetchOpDimension::k1D: {
              ...
              gradient_h_x = builder_->createUnaryOp(spv::OpDPdxCoarse, type_float_, coordinates[0]);
              gradient_v_x = builder_->createUnaryOp(spv::OpDPdyCoarse, type_float_, coordinates[0]);
              // For wide 1D textures, also compute Y gradients.
              // coordinates[1] is non-zero only for wide 1D.
              gradient_h_y = builder_->createUnaryOp(spv::OpDPdxCoarse, type_float_, coordinates[1]);
              gradient_v_y = builder_->createUnaryOp(spv::OpDPdyCoarse, type_float_, coordinates[1]);
