...
// the real XMPSetVolume export - confirmed necessary for Project Gotham
// Racing 3/4, which persist volume through their own save format and only
// ever call XMPSetVolume(0.0) once at boot (see AudioMediaPlayer::PlaySong's
// separate "treat persistent 0.0 as unset" workaround, which this patch is
// the real fix for on titles it covers). Each game just needs to specify:
...
// buffer and re-reads it on controller input polls to detect in-game volume
// adjustments before they are saved.
//
// Ported from xenia-canary commit 02fccd9 ("[Kernel/XMP] Add title-specific
// file I/O hooks for XMP volume sync").
...
class XmpVolumePatch {
 public:
  ...
  void OnFileWrite(const std::string& path, const uint8_t* buffer, uint32_t length,
                   uint32_t guest_address);
  void OnFileRead(const std::string& path, const uint8_t* buffer, uint32_t length,
                  uint32_t guest_address);
  void OnInputPoll(uint32_t packet_number);

  // Returns a patch for `title_id`, or nullptr if this title isn't one of
  // the known games that need one.
  static std::unique_ptr<XmpVolumePatch> CreateForTitle(uint32_t title_id, KernelState* ks);
